 ...
 * on newyorkcoin testnet and won't be accepted by newyorkcoind running
 * on the main newyork blockchain (real newyork addresses start with D).
 */
...
async.waterfall([
    function(next) {
      ...
      var outputs = {};
      /* Leave one NYC unspent as transaction fee. */
    ...
    function(unsigned_hex, next) {
      ...
      newyorkcoin.signRawTransaction(unsigned_hex, [], [tx.private_key], next);
    },
    ...
    function(signed_hex, next) {
      ...
      newyorkcoin.decodeRawTransaction(signed_hex, next);
    }
